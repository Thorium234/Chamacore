"""C2B (manual Paybill) validation + confirmation tests (ADR-016, OQ-021).

These exercise the real Daraja parser and the fail-closed service/endpoint
behaviour: validation always rejects until a reference-matching rule exists,
confirmation is stored idempotently and acknowledged, and neither path writes
to the ledger (blocked by OQ-012/OQ-013 + OQ-021).
"""
...
class TestC2BValidation:
    def test_validation_rejects_fail_closed_and_records_event(self, client, db):
        _, _, _, urls = _setup(client)
        r = _post(client, urls["validate"], C2B_PAYLOAD)
        assert r.status_code == 200
        assert r.json()["ResultCode"] == 1
        assert "OQ-021" in r.json()["ResultDesc"]
        events = _events(db)
        assert len(events) == 1
    ...
    def test_validation_invalid_token_rejects_without_event(self, client, db):
        ...
        r = _post(client, url, C2B_PAYLOAD)
        assert r.status_code == 200
        assert r.json()["ResultCode"] == 1
        assert _events(db) == []
    ...
    def test_validation_rejects_even_with_matching_shortcode(self, client, db):
        _, _, _, urls = _setup(client)
        r = _post(client, urls["validate"], C2B_PAYLOAD)
        assert r.status_code == 200
        assert r.json()["ResultCode"] == 1
...
class TestC2BConfirmation:
    def test_confirmation_stores_event_and_acknowledges(self, client, db):
        _, _, _, urls = _setup(client)
        r = _post(client, urls["confirm"], C2B_PAYLOAD)
        assert r.status_code == 200
        data = r.json()
    ...
    def test_confirmation_duplicate_delivery_is_deduplicated(self, client, db):
        _, _, _, urls = _setup(client)
        first = _post(client, urls["confirm"], C2B_PAYLOAD).json()
        second = _post(client, urls["confirm"], C2B_PAYLOAD).json()
    ...
    def test_confirmation_same_trans_id_different_body_disagrees(self, client, db):
        _, _, _, urls = _setup(client)
        altered = dict(C2B_PAYLOAD, TransAmount="999")
        _post(client, urls["confirm"], C2B_PAYLOAD)
        r = _post(client, urls["confirm"], altered)
        assert r.json()["event_status"] == "DISAGREEMENT"
        assert len(_events(db)) == 1

    def test_confirmation_shortcode_mismatch_is_unprocessable(self, client, db):
        _, _, _, urls = _setup(client)
        foreign = dict(C2B_PAYLOAD, BusinessShortCode="999999")
        r = _post(client, urls["confirm"], foreign)
        assert r.status_code == 200
